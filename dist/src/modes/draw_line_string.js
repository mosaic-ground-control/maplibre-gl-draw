...
export class DrawLineString extends ModeBase {
    onSetup(opts) {
        opts = opts || {};
        ...
        if (featureId) {
            ...
            if (line.coordinates[lastCoord][0] === from[0] &&
                line.coordinates[lastCoord][1] === from[1]) {
                currentVertexPosition = lastCoord + 1;
                // add one new coordinate to continue from
                line.addCoordinate(currentVertexPosition, ...line.coordinates[lastCoord]);
            }
            else if (line.coordinates[0][0] === from[0] &&
    ...
    onClick(state, e) {
        if (CommonSelectors.isVertex(e))
            return this.clickOnVertex(state);
    ...
    toDisplayFeatures(state, geojson, display) {
        ...
        display(createVertex(state.line.id, geojson.geometry.coordinates[state.direction === "forward"
...
//# sourceMappingURL=draw_line_string.js.map