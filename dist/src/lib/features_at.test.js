            ...
            if (params && params.layers) {
                for (const layerId of params.layers) {
                    ...
                    if (!layer) {
                        ...
                        throw new ErrorEvent(`The layer '${layerId}' does not exist in the map's style and cannot be queried for features.`);
                    }
...
test("featuresAt with click bounding box", () => {
    ...
    const result = featuresAt.click(null, [
        [10, 10],
        [20, 20],
    ], mockContext);
    assertEquals(result, [
...
test("featuresAt with touch bounding box", () => {
    const mockContext = createMockContext();
    const result = featuresAt.touch(null, [
        [10, 10],
        [20, 20],
    ], mockContext);
    assertEquals(result, [
...
test("featuresAt should not include missing style layers", () => {
    ...
    let result = featuresAt.touch(null, [
        [10, 10],
        [20, 20],
    ], mockContext);
    assertEquals(result, [], "sorts, filters based on properties.meta, removes duplicates");
...
//# sourceMappingURL=features_at.test.js.map